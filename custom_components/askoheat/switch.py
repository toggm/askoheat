...
class AskoheatAutoFeedInSwitch(AskoheatSwitch):
    ...
    def __init__(  # noqa: PLR0913
        ...
        *,
        ...
        LOGGER.debug("Track buffer entity:%s", self._buffer_entity_id)

        self.async_on_remove(
            async_track_state_change_event(
                hass,
                [power_entity_id, self._buffer_entity_id],
        ...
        match event.data["entity_id"]:
            case self._buffer_entity_id:
                if new_state and new_state.state != "unknown":
                    ...
                    current_power_value = self._hass.states.get(self._power_entity_id)
